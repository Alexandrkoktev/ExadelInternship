...
import { mapStateToProps, mapDispatchToProps } from '../../commands/notifications'
...
class Message extends React.Component {
    ...
    render() {
        const { routeMessage, bookingMessage, ride, passengers, driver } = this.props
        return (
            <>
...
class SendMessage extends React.Component {
    ...
    componentDidMount() {
        if (this.props.ride)
            this.setState({ chosen: [this.props.to.id] })
    }
    ...
    render() {
        ...
        return (
            <Modal centered show={show} onHide={hide}>
                <Modal.Header><Modal.Title>Write your message</Modal.Title></Modal.Header>
                <Modal.Body>
                    <Form>
                        <Form.Group>
                            <Form.Label>Message will be sent to</Form.Label>{' '}
                            {addressant}
                        </Form.Group>
                        <Form.Label>Your Message</Form.Label>
                        <Form.Control as="textarea" rows="3" onChange={this.handleChange} value={this.state.message} />
                    </Form>
                </Modal.Body>
                <Modal.Footer>
                    <Button variant='info' onClick={click}>Send</Button>
                </Modal.Footer>
            </Modal >
        )
    }
